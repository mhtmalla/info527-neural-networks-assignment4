...
Original file is located at
    https://colab.research.google.com/drive/1AQVPK6Tay0AJdAntD5F3E9oMiywmOwkU
"""
...
def test_toy_rnn(capsys):
    ...
    # Normalize input data to the range [0.1, 1.0]
    train_in = train_in / 10.0
    test_in = test_in / 10.0
    ...
    # Normalize output data to match the input scaling
    train_out = train_out / 10.0
    ...
    # make predictions and scale back to original range
    predictions = model.predict(test_in) * 10.0
    true_outputs = test_out * 10.0

    # make sure error is low enough
    rmse = root_mean_squared_error(predictions, true_outputs)
...
def loss_name(model):
    if isinstance(model.loss, str):
        loss_name_str = model.loss.lower()
    elif hasattr(model.loss, '__name__'):
        loss_name_str = model.loss.__name__.lower()
    elif hasattr(model.loss, 'name'):
        loss_name_str = model.loss.name.lower()
    else:
        loss_name_str = ''
    return loss_name_str
